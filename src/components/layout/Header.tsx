...
export function Header({ title, subtitle, onMenuClick, showMenuButton }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-40 bg-white border-b border-[#EDEDED] px-4 md:px-8 py-4 shadow-sm">
      <div className="flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-xl md:text-2xl font-bold tracking-tight truncate text-[#111111]">{title}</h1>
            {subtitle && (
              <p className="text-xs md:text-sm text-[#666666] mt-0.5 truncate hidden sm:block">
                {subtitle}
              </p>
            )}
          </div>
          ...
          <Popover open={notifOpen} onOpenChange={setNotifOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                variant="ghost"
                size="icon"
                className="relative h-9 w-9 md:h-10 md:w-10 rounded-full bg-[#F8F8F5] hover:bg-[#EAEAEA] text-[#111111] transition-colors"
              >
